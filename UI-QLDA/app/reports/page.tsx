...
const chartColors = [
  "#2C6E49", "#F4A261", "#0DCAF0", "#6C757D",
  "#8B5CF6", "#198754", "#DC3545", "#856404",
];
...
export default function ReportsPage() {
  ...
  const chartData = branchReportData.map(b => ({
    name: b.TenCN.replace("Phụng Lộc Coffee - ", ""),
    DoanhThu: b.DoanhThu,
  ...
  // ... (Gắn lại hàm handleExportExcel và handleExportPDF của bạn y hệt như cũ, mình rút gọn để code không quá dài)
    ...
    <MainLayout title="Báo cáo doanh thu" breadcrumb="Trang chủ / Báo cáo / Doanh thu">
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-4 rounded-lg bg-card p-4 shadow-[0_2px_8px_rgba(0,0,0,0.08)]">
          ...
          <Button variant="outline" className="gap-2" onClick={fetchReportData}><RefreshCw className="h-4 w-4" /> Làm mới</Button>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <div className="rounded-lg bg-card p-5 shadow-[0_2px_8px_rgba(0,0,0,0.08)]">
            ...
            <p className="mt-2 truncate text-2xl font-bold text-primary">{bestBranch ? bestBranch.TenCN : "—"}</p>
          </div>
        </div>
          ...
          <div className="h-[350px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                ...
                <XAxis dataKey="name" tick={{ fontSize: 12, fill: "#6C757D" }} axisLine={{ stroke: "#E9ECEF" }} />
                <YAxis tick={{ fontSize: 12, fill: "#6C757D" }} axisLine={{ stroke: "#E9ECEF" }} tickFormatter={formatCompact} />
                <Tooltip formatter={(value) => formatCurrency(Number(value))} contentStyle={{ backgroundColor: "#fff", borderRadius: "8px" }} />
                <Bar dataKey="DoanhThu" radius={[4, 4, 0, 0]}>
                  {chartData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={entry.MauBieuDo} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
