...
DECLARE
    ...
    v_ngay_bat_dau DATE := CURRENT_DATE - INTERVAL '90 days';
    
    v_macn VARCHAR(10);
    v_manv_banhang VARCHAR(10);
    v_manv_kho VARCHAR(10);
    v_maca VARCHAR(50);
    v_mahd VARCHAR(50);
    v_makk VARCHAR(50);
    v_masp VARCHAR(10);
    ...
    v_so_luong_hd INT;
    v_tong_tien_hd NUMERIC(15,2);
    v_gia_ban NUMERIC(15,2);
    
    i INT; j INT; k INT; l INT;
    
    -- Danh sách chi nhánh hợp lệ từ file gốc của bạn
    v_ds_chinhanh VARCHAR[] := ARRAY['CN01', 'CN02', 'CN03', 'CN04'];
    v_ds_nguyenlieu VARCHAR[] := ARRAY['NL001', 'NL002', 'NL003', 'NL004', 'NL005', 'NL006', 'NL007'];
    
BEGIN
    -- Vòng lặp quét qua từng ngày trong 90 ngày qua
    FOR i IN 0..90 LOOP
        v_ngay_hien_tai := v_ngay_bat_dau + (i || ' days')::INTERVAL;
        
        -- Quét qua 4 chi nhánh có nhân viên
        FOR j IN 1..4 LOOP
            v_macn := v_ds_chinhanh[j];
            
            -- 1. Mapping đúng nhân viên theo chi nhánh (Dựa trên data gốc của Khoa)
            IF v_macn = 'CN01' THEN 
                v_manv_banhang := 'NV003'; v_manv_kho := 'NV005'; v_so_luong_hd := floor(random() * 30 + 40); -- Q1 bán 40-70 đơn/ngày
            ELSIF v_macn = 'CN02' THEN 
                v_manv_banhang := 'NV007'; v_manv_kho := 'NV007'; v_so_luong_hd := floor(random() * 20 + 20); -- Q3 bán 20-40 đơn/ngày
            ELSIF v_macn = 'CN03' THEN 
                v_manv_banhang := 'NV009'; v_manv_kho := 'NV010'; v_so_luong_hd := floor(random() * 15 + 15); -- BT bán 15-30 đơn/ngày
            ELSE 
                v_manv_banhang := 'NV011'; v_manv_kho := 'NV011'; v_so_luong_hd := floor(random() * 10 + 10); -- TĐ bán 10-20 đơn/ngày
            END IF;

            -- 2. TẠO CA LÀM VIỆC MỖI NGÀY CHO CHI NHÁNH (Đã xóa cột trangthai)
            v_maca := 'CA_AUTO_' || v_macn || '_' || TO_CHAR(v_ngay_hien_tai, 'YYYYMMDD');
            INSERT INTO calamviec (maca, manv, macn, thoigianmo, thoigiandong, tiendauca, issynced, createdat) 
            VALUES (v_maca, v_manv_banhang, v_macn, v_ngay_hien_tai + INTERVAL '07:00:00', v_ngay_hien_tai + INTERVAL '22:00:00', 1000000, TRUE, v_ngay_hien_tai + INTERVAL '07:00:00')
            ON CONFLICT DO NOTHING;

            -- 3. TẠO HÓA ĐƠN VÀ CHI TIẾT
            FOR k IN 1..v_so_luong_hd LOOP
                v_mahd := 'HD_AUTO_' || v_macn || '_' || TO_CHAR(v_ngay_hien_tai, 'YYMMDD') || '_' || LPAD(k::TEXT, 4, '0');
                v_tong_tien_hd := 0;
                
                -- Tạo Hóa đơn (Đã khớp số lượng cột với data chuẩn của bạn)
                INSERT INTO hoadon (mahd, maca, macn, tongtien, trangthai, issynced, createdat)
                VALUES (v_mahd, v_maca, v_macn, 0, 1, TRUE, v_ngay_hien_tai + (random() * 14 + 7 || ' hours')::INTERVAL)
                ON CONFLICT DO NOTHING;

                -- Bơm 1-3 món ăn cho mỗi hóa đơn (CƠ CHẾ PHÂN LOẠI MÓN BÁN CHẠY)
                FOR l IN 1..floor(random() * 3 + 1) LOOP
                    IF v_macn = 'CN01' THEN
                        v_masp := 'SP00' || floor(random() * 3 + 1)::TEXT; 
                    ELSIF v_macn = 'CN02' THEN
                        v_masp := 'SP00' || floor(random() * 3 + 4)::TEXT;
                    ELSE
                        v_masp := 'SP00' || floor(random() * 7 + 1)::TEXT;
                    END IF;

                    -- Lấy giá hiện tại
                    SELECT giahientai INTO v_gia_ban FROM sanpham WHERE masp = v_masp;
                    IF v_gia_ban IS NULL THEN v_gia_ban := 30000; END IF;

                    -- Insert CTHD (Thêm NULL cho idmonchinh và ghichu để khớp cấu trúc)
                    INSERT INTO cthd (id, mahd, masp, idmonchinh, soluong, giabantaithoidiem, ghichu)
                    VALUES (v_mahd || '_' || l, v_mahd, v_masp, NULL, floor(random() * 3 + 1), v_gia_ban, NULL)
                    ON CONFLICT DO NOTHING;
                END LOOP;

                -- Tính lại tổng tiền
                SELECT COALESCE(SUM(soluong * giabantaithoidiem), 0) INTO v_tong_tien_hd FROM cthd WHERE mahd = v_mahd;
                UPDATE hoadon SET tongtien = v_tong_tien_hd WHERE mahd = v_mahd;

                -- Thanh toán
                INSERT INTO thanhtoan (matt, mahd, phuongthuc, sotien, trangthai, issynced, createdat)
                VALUES ('TT_' || v_mahd, v_mahd, CASE WHEN random() > 0.5 THEN 'MOMO' ELSE 'TIENMAT' END, v_tong_tien_hd, 1, TRUE, v_ngay_hien_tai + INTERVAL '12:00:00')
                ON CONFLICT DO NOTHING;
            END LOOP;
            
            -- 4. TẠO PHIẾU KIỂM KHO (Mỗi tuần kiểm 1 lần vào Chủ nhật)
            IF EXTRACT(DOW FROM v_ngay_hien_tai) = 0 THEN
                v_makk := 'KK_AUTO_' || v_macn || '_' || TO_CHAR(v_ngay_hien_tai, 'YYYYMMDD');
                
                INSERT INTO kiemkho (makk, manv, macn, ngaykiem, issynced, createdat)
                VALUES (v_makk, v_manv_kho, v_macn, v_ngay_hien_tai + INTERVAL '22:30:00', TRUE, v_ngay_hien_tai + INTERVAL '22:30:00')
                ON CONFLICT DO NOTHING;
                
                -- Bơm chi tiết kiểm kho (3 món ngẫu nhiên)
                FOR l IN 1..3 LOOP
                    INSERT INTO ctkk (makk, manl, soluonghethong, soluongthucte, chenhlech)
                    VALUES (
                        v_makk, 
                        v_ds_nguyenlieu[floor(random() * 7 + 1)], 
                        round((random() * 5000 + 1000)::numeric, 2), 
                        round((random() * 5000 + 800)::numeric, 2), 
